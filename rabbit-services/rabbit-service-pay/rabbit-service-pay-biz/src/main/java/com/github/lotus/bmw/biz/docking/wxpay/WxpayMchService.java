...
public class WxpayMchService implements PaymentMchService {
    ...
    public PayTradeVo goPay(PayTradeRo ro) {
        ...
        GoPayRequest goPayRequest = GoPayRequest.builder()
            .quitUrl(tradeOrder.getFrontJumpUrl())
            .payAmount(tradeOrder.getTradeAmt())
            .tradeSn(orderNo).build();
        String urlString = goPayRequest.getClass().getSimpleName();

        String result = httpLogBervice.call(() -> {
            GoPayResponse response = goPayRequest.request(ro.getPayType());

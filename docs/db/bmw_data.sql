-- 基础数据
INSERT INTO bmw_payment_mch_type (encoding, title, enabled)
VALUES ('alipay', '支付宝', 1);
set @mch_type_id := last_insert_id();
INSERT INTO bmw_payment_mch_pay_type (payment_mch_type_id, encoding, title, enabled)
VALUES (@mch_type_id, 'alipay.pc', '支付宝(PC)', 1);
INSERT INTO bmw_payment_mch_pay_type (payment_mch_type_id, encoding, title, enabled)
VALUES (@mch_type_id, 'alipay.native', '支付宝(Native)', 1);
INSERT INTO bmw_payment_mch_pay_type (payment_mch_type_id, encoding, title, enabled)
VALUES (@mch_type_id, 'alipay.app', '支付宝(APP)', 1);
INSERT INTO bmw_payment_mch_pay_type (payment_mch_type_id, encoding, title, enabled)
VALUES (@mch_type_id, 'alipay.qrcode', '支付宝(QrCode)', 1);

INSERT INTO bmw_payment_mch_type (encoding, title, enabled)
VALUES ('wxpay', '微信支付', 1);
set @mch_type_id := last_insert_id();
INSERT INTO bmw_payment_mch_pay_type (payment_mch_type_id, encoding, title, enabled)
VALUES (@mch_type_id, 'wxpay.jsapi', '微信支付(JSAPI)', 1);
INSERT INTO bmw_payment_mch_pay_type (payment_mch_type_id, encoding, title, enabled)
VALUES (@mch_type_id, 'wxpay.app', '微信支付(APP)', 1);
INSERT INTO bmw_payment_mch_pay_type (payment_mch_type_id, encoding, title, enabled)
VALUES (@mch_type_id, 'wxpay.native', '微信支付(Native)', 1);

-- 接入商户
insert into bmw_access_mch(id, encoding, title, created_at)
    value (1, 'test_access', '测试应用', now());

-- 支付商户
insert into bmw_payment_mch(id, encoding, type, created_at)
    value (1, 'test_payment', 'alipay', now());
insert into bmw_payment_mch(id, encoding, type, created_at)
    value (2, 'test_payment_2', 'wxpay', now());

-- bmw_payment_mch_alipay
insert into bmw_payment_mch_alipay_config(id, appid, public_key, private_key)
    value (1, '2016080300154586',
           '',
           '');
INSERT INTO bmw_payment_mch_wxpay_config (id, appid, mch_id, key_str, cert_str)
VALUES (2, 'wxffc9e352055986fb', '1576760381', 'quanqiugouwechatpay15951onyidali', '-----BEGIN CERTIFICATE-----
              
              -----END CERTIFICATE-----');

-- 授权接入商户
insert into bmw_payment_access_ref(access_mch_id, payment_mch_id)
VALUES (1, 1),
       (1, 2);

-- 支付商户授权场景码
INSERT INTO bmw_pay_scene(access_mch_id, encoding, title, created_at)
    value (1, 'test_scene', '测试场景', now());
set @last_id := last_insert_id();
INSERT INTO bmw_pay_scene_support(pay_scene_id, payment_mch_id, pay_type)
VALUES (@last_id, 1, 'alipay.pc'),
       (@last_id, 1, 'alipay.native'),
       (@last_id, 1, 'alipay.app'),
       (@last_id, 1, 'alipay.qrcode'),
       (@last_id, 2, 'wxpay.jsapi'),
       (@last_id, 2, 'wxpay.app'),
       (@last_id, 2, 'wxpay.native');

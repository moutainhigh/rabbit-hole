...
public class LoginRo {
    @NotNull(message = "该登录方式暂不支持", groups = {PasswordMode.class, SmsMode.class})
    @EnumRange(message = "该登录方式暂不支持", groups = {PasswordMode.class, SmsMode.class}, enumClass = Mode.class)
    @ApiModelProperty(value = "模式", required = true)
    private String mode = Mode.UsePassword.getCode();

    @Valid
    @NotNull(message = "手机信息不能为空", groups = {SmsMode.class})
    @ApiModelProperty(value = "仅短信模式使用")
    private SmsMode smsMode;

    @Valid
    @NotNull(message = "账号信息不能为空", groups = {PasswordMode.class})
    ...
    public static class PasswordMode {
        @NotBlank(message = "账户不能为空", groups = {PasswordMode.class})
        @ApiModelProperty("账号/手机号/邮箱")
        private String username;
        @NotBlank(message = "密码不能为空", groups = {PasswordMode.class})
        @ApiModelProperty("密码")
    ...
    public static class SmsMode {
        @Pattern(message = "请输入正确的手机号码", groups = {SmsMode.class}, regexp = RegexpConstant.PHONE)
        @NotBlank(message = "手机号码不能为空", groups = {SmsMode.class})
        @ApiModelProperty("手机号")
        private String phone;
        @Pattern(message = "请输入6位验证码", groups = {SmsMode.class}, regexp = RegexpConstant.VERIFY_CODE)
        @NotBlank(message = "验证码不能为空", groups = {SmsMode.class})
